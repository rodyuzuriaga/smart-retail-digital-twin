...
// import logo from '../logo.jpeg'; // Removed import, use static path

export const Sidebar: React.FC<SidebarProps> = ({ activeView, onViewChange }) => {
  const menuItems = [
    { id: ViewType.DASHBOARD, label: 'Dashboard General', icon: LayoutDashboard },
    { id: ViewType.WAREHOUSE_MAP, label: 'Gemelo Digital', icon: Binary },
    ...
    <aside className="w-64 bg-white border-r border-gray-100 flex flex-col z-20">
      <div className="p-10 pb-4">
        <div className="flex flex-col select-none group cursor-default">
          <img src="/logo.jpeg" alt="Tailoy Logo" className="w-24 object-contain" />
      ...
      <div className="p-6 border-t border-gray-50 bg-gray-50/30">
        <button 
            ...
            className="flex items-center space-x-3 px-4 py-2 w-full text-gray-400 hover:text-red-500 transition-colors text-[12px] font-bold uppercase tracking-wider"
        >
          <LogOut size={16} />
          <span>Volver al 3D</span>
        </button>
      </div>
    </aside>
  );
